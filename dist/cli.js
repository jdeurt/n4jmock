...
const args = await yargs(process.argv.slice(2))
    ...
    .number("nodes")
    .alias("nodes", ["n"])
    .describe("nodes", "The amount of unique nodes to generate for each label (Foo {})")
    .number("relationships")
    .alias("relationships", ["r"])
    .describe("relationships", "The number of connections to generate for each relationship field (foo -> bar)")
    .string("out")
    .alias("out", ["o"])
    .describe("out", "The location the resulting query should be written to")
    .boolean("nofile")
    .describe("nofile", "Do not generate an output file and instead write the result to stdout")
    .help().argv;
if (!args._[0]) {
    throw new Error("Entry point not provided. This should've been already handled yet here we are.");
}
